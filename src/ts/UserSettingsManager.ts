module Player {
	...
	export class UserSettingsManager {
		...
		public constructor() {
			...
			if (configExists) {
				try {
					this._userSettings = this.userSettingsFromString(configData, parseInt(configDataVersion, 10));
				} catch (e) {
		...
		public reset() {
			window.localStorage.setItem(UserSettingsManager.LOCAL_STORAGE_VERSION_KEY, null);
