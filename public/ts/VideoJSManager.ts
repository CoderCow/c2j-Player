...
videojs.registerComponent('MouseTimeDisplay', DummyComponent);
videojs.registerComponent('CurrentTimeDisplay', DummyComponent);
videojs.registerComponent('CaptionsButton', DummyComponent);
...
export class VideoJSManager {
	...
	public init(initCompleted: (error: Exception | undefined) => void): void {
		var userPreferredLanguage = Utils.normalizeLanguageCode(navigator.language || navigator.userLanguage || navigator.browserLanguage || navigator.systemLanguage || 'de');
		...
		var manager = this;
		videojs(VideoJSManager.VIDEOJS_ID, videojsOptions, function(this: VideoJSPlayer) {
			var error: Exception | undefined;
			try {
				manager._player = this;

				// Set playback position prior loading media, so no useless buffering will be done.
				if (manager._playerConfig.startPlaybackAt !== undefined)
				...
				var initialSubtitleLanguage: string | undefined;
	 ...
	 */
	public useVideoData(fromUrl: string, loadCompleted: (error: Exception | undefined) => void) {
	...
	private preprendMetadataUrl(url: string): string {
		if (!Utils.isAbsoluteUrl(url) && this._playerConfig.videoMetaBaseUrl !== null)
	...
	private prependMediaUrl(url: string): string {
		if (!Utils.isAbsoluteUrl(url) && this._playerConfig.videoMediaBaseUrl !== null)
